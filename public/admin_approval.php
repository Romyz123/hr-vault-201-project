    ...
    foreach ($reqIds as $current_req_id) {
        ...
        if ($coreAction === 'approve') {
            try {
                ...
                $pdo->prepare("UPDATE requests SET status = 'APPROVED', admin_comment = ? WHERE id = ?")->execute(["Approved by " . htmlspecialchars($_SESSION['username'] ?? 'Admin', ENT_QUOTES, 'UTF-8'), $current_req_id]);
...
function renderTable($requests, $type)
{
    ...
    foreach ($requests as $r) {
        ...
        elseif ($type == 'doc_edit') {
            ...
            $summary = "<strong class='text-primary'><i class='bi bi-check2-circle'></i> Action Taken{$docLabel}:</strong> " . htmlspecialchars(substr((string)($data['note'] ?? ''), 0, 100), ENT_QUOTES, 'UTF-8') . "...";
        }
    ...
    function openPreview(data, type, reqId = null) {
        ...
        // 3. PROFILE ADD / EDIT
            ...
            if (type === 'edit') {
                ...
                const skipKeys = ['id', 'avatar_path', 'updated_at', 'created_at', 'request_note', 'import_batch', 'system_role'];
                                    ...
                                    <td>
                                        <div class="d-flex gap-3 align-items-center">
                                            <div class="text-center">
                                                ...
                                                <img src="uploads/avatars/${escapeHtml(oldAv)}" class="rounded-circle border" style="width:50px; height:50px; object-fit:cover;">
                                            </div>
                                            <i class="bi bi-arrow-right fs-4"></i>
                                            <div class="text-center">
                                                <small class="text-success fw-bold d-block">New</small>
                                                <img src="uploads/avatars/${escapeHtml(newAv)}" class="rounded-circle border border-success" style="width:50px; height:50px; object-fit:cover;">
                                            </div>
                                        </div>
                                    </td>
                                </tr>`;
