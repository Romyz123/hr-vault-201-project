...
if ($isWordExport) {
    ...
    $avatarPath = __DIR__ . '/uploads/avatars/' . ($emp['avatar_path'] ?: 'default.png');
    $avatarSrc = file_exists($avatarPath) ? 'data:image/' . pathinfo($avatarPath, PATHINFO_EXTENSION) . ';base64,' . base64_encode(file_get_contents($avatarPath)) : '';
} else {
    $avatarSrc = 'uploads/avatars/' . htmlspecialchars($emp['avatar_path'] ?: 'default.png');
}
?>
        ...
        <div class="container py-3 no-print">
            <div class="p-3 bg-light border rounded shadow-sm d-flex flex-column flex-md-row justify-content-between align-items-md-center gap-3">
                ...
                <div class="text-end text-nowrap">
                    ...
                    <a href="?id=<?php echo $id; ?>&export=word" class="btn btn-primary fw-bold shadow-sm ms-1"><i class="bi bi-file-word"></i> Export to Word</a>
                    <a href="index.php" class="btn btn-secondary shadow-sm ms-1">Back</a>
                </div>
            </div>
        </div>
    <?php endif; ?>
