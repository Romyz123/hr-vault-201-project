...
// [SECURITY FIX] Move error log OUTSIDE the web root to prevent public access to stack traces and DB paths
$logDir = 'C:/xampp/logs/';
if (!is_dir($logDir)) {
    // Fallback to parent of DOCUMENT_ROOT if xampp/logs doesn't exist
    $logDir = dirname(dirname(__DIR__)) . DIRECTORY_SEPARATOR . 'logs' . DIRECTORY_SEPARATOR;
    if (!is_dir($logDir)) @mkdir($logDir, 0750, true);
}
ini_set('error_log', $logDir . 'hr_vault_errors.log');
...
// [SECURITY FIX] Single authoritative CSP — includes QR code CDN and Bootstrap Icons
...
function enforceSecurityQuestionSetup($pdo, $ignoreWhitelist = false)
{
    ...
    try {
        ...
        if (empty($question)) {
            // Disabled forced redirect per user request
